...
Date creation    12 July 2024

Script that can be run on a daily basis to download, process
and analyze the total precipitation in some location
"""
...
# Downloading ERA5 data total precip
...
for locationName in locationNames:
	...
	for year in np.arange(startYear, endYear):
		fileYear = "../data/download_precip_" + str(locationName) + "_" + str(year) + ".nc"
		...
		try:
			...
			# Special case where precip has an extra dimension of length 2
			# and the data has to be fetched accordingly. Sometimes it 
			# is in the first, sometimes in the second dimension of 
			# that variable

			fillValue= f.variables["tp"]._FillValue

			precip_1 = np.squeeze(f.variables["tp"][:, 0, :, :]).data 
			precip_2 = np.squeeze(f.variables["tp"][:, 1, :, :]).data 

			precip_1[precip_1 == fillValue] = np.nan
			precip_2[precip_2 == fillValue] = np.nan

			thisData = np.nanmean(np.array((precip_1, precip_2)), axis=0) * scalef

		except KeyError:

			# If data is organized normally
			thisData = np.squeeze(f.variables["tp"][:]).data       * scalef
	...
	if endMonth == 1:
		...
		listTime   = [str(j).zfill(2) + ":00" for j in np.arange(24)]
		fileOut    = "../data/download_precip_" + str(locationName) + "_" + str(endYear) + "-" + str(endMonth).zfill(2) + ".nc"
		...
		for m in np.arange(1, endMonth):
			...
			listDays   = [str(d).zfill(2) for d in np.arange(1, 31 + 1)]
			listTime   = [str(j).zfill(2) + ":00" for j in np.arange(24)]
			fileOut = "../data/download_precip_" + str(locationName) + "_" + str(endYear) + "_" + str(m).zfill(2) + ".nc"
		...
		listDays   = [str(d).zfill(2) for d in np.arange(1, endDay + 1)]
		listTime   = [str(j).zfill(2) + ":00" for j in np.arange(24)]
		fileOut = "../data/download_precip_" + str(locationName) + "_" + str(endYear) + "_" + str(endMonth).zfill(2) + "_" + str(1).zfill(2) + "-" + str(endDay).zfill(2) + ".nc"
		downloadERA5(endYear, listMonths, listDays, listTime, domainArea, outFile = fileOut)
	...
	outCSV = "../output/hourly_precip_" + locationName + ".csv"
	
	with open(outCSV, "w") as csvFile:
		csvFile.write("# Précipitations cumulées, fréquence horaire, à " + locationName + " (données ERA5)\n") 
		csvFile.write("AAAA-MM-JJ hh-mm-ss, precip (mm)\n")
	...
	outCSV = "../output/dailyStatistics_precip_" + locationName + ".csv"
	
	with open(outCSV, "w") as csvFile:
		csvFile.write("# Statistiques journalières des précipitations cumulées à " + locationName + " (données ERA5)\n") 
	...
	if locationName == "Bruxelles" and doAddLatestData:
	    ...
	    datesDayListPrecip = list()
	    dataDaySumListPrecip = list()
	    ...
	    for j, row in enumerate(reader):
	            if j < 10:
	                ...
	                if thisDate > datesDay[-1]:
	                    datesDayListPrecip.append(thisDate)
	                    dataDaySumListPrecip.append(row[4])
	 
	    # Append
	    [datesDay.append(     d)      for d in datesDayListPrecip[   -1::-1]]
	    [dataDaySum.append(float(d)) for d in dataDaySumListPrecip[-1::-1 ]]
	    [flagDayData.append("RMI")    for d in datesDayListPrecip[-1::-1]]
